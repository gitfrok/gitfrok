...
| 14 | `helm lint` / `helm template` render the install (required values enforced by `required`) | all | `scripts/check-byo-chart.sh` — runs only when `helm` is on PATH; where it is not, the gate says so on its own output line (the authoring machine of this row had no helm: static assertions only). **Ran:** `helm lint` and both `helm template` renders (default and `operator.enabled=true` with zero image values) executed green on a helm-equipped lane 2026-08-21 (helm v4.2.4) | not run |
